...
struct EffectsView: View {
        ...
        return ZStack {
            ...
            GeometryReader { geo in
                Canvas { context, size in
                    ...
                    let teal = Color(red: 0, green: 0.83, blue: 0.67)

                    // Build ordered nodes
                    struct Node { var point: CGPoint; var active: Bool }
                    var nodes: [Node] = []
                    for item in items {
                        guard let pos = blockPositions[item.id] else { continue }
                        let active: Bool
                        switch item.kind {
                        case .terminal: active = true
                        case .effect(let idx): active = stages[idx].enabled
                        }
                        nodes.append(Node(point: pos, active: active))
                    }
                    guard nodes.count >= 2 else { return }

                    // Draw one cable per segment. Active = solid teal with glow + dots.
                    // Bypassed destination = dashed, dimmer, no dots.
                    for i in 0..<(nodes.count - 1) {
                        let from = nodes[i].point
                        let to = nodes[i + 1].point
                        let bothActive = nodes[i].active && nodes[i + 1].active
                        ...
                        if sameRow {
                            ...
                            let r: CGFloat = 12
                            cablePath.move(to: from)
                            cablePath.addLine(to: CGPoint(x: midX - r, y: from.y))
                            cablePath.addQuadCurve(to: CGPoint(x: midX, y: from.y + r),
                                                   control: CGPoint(x: midX, y: from.y))
                            cablePath.addLine(to: CGPoint(x: midX, y: to.y - r))
                            cablePath.addQuadCurve(to: CGPoint(x: midX - r, y: to.y),
                                                   control: CGPoint(x: midX, y: to.y))
                            cablePath.addLine(to: to)
                        }

                        if bothActive {
                            // Solid cable with glow
                            context.stroke(cablePath, with: .color(teal.opacity(0.12)), lineWidth: 6)
                            context.stroke(cablePath, with: .color(teal.opacity(0.5)), lineWidth: 2)
                            // Signal dots
                            ...
                            // Dashed cable, dimmer
                            context.stroke(cablePath, with: .color(teal.opacity(0.18)),
    ...
    /// Cubic bezier interpolation for bypass arc dot positions
